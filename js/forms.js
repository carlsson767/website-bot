  ...
  forms.forEach((form) => {
    form.addEventListener('submit', (event) => {
      ...
      .then(response => {
         if (!response.ok) {
             return response.json()
               .then((body) => { throw new Error(body.error || body.message || 'Ошибка сервера'); })
               .catch(() => { throw new Error('Ошибка связи с сервером'); });
         }
         return response.json();
      })
      .then(result => {
      ...
      .catch(error => {
        ...
        showFormMessage(form, error.message || 'Ошибка связи с сервером. Попробуйте позже или позвоните нам.', 'error');
      })
      .finally(() => {
